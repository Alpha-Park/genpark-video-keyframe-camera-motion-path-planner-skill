# genpark-video-keyframe-camera-motion-path-planner-skill
...
**genpark-video-keyframe-camera-motion-path-planner-skill** is a deterministic, zero-dependency Python skill engineered for autonomous design agents, spatial canvas reasoning, and multimodal UI generation pipelines.

> **Executive Capability**: Generative video keyframe 3D camera trajectory spline planner (Runway Gen-3 / Luma)
...
result = client.plan_camera_motion_path()
print(result)
```
...
```json
{
  "mcpServers": {
    "genpark-video-keyframe-camera-motion-path-planner-skill": {
      "command": "python",
      "args": ["/path/to/genpark-video-keyframe-camera-motion-path-planner-skill/mcp_server.py"]
    }
  }
}
```
